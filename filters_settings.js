...
export function apply_filters(filter_input, options, results_section) {
    ...
    if (!coincidencia) {
        GRID_PRODUCTS.style.setProperty(
...
export function get_filters_settings() {
    function evaluation(section) {
        if (
            section.classList.contains("brand_label") ||
            section.classList.contains("filter_label") ||
            section.classList.contains("results")
        ) {
            return false;
        }
        return true;
...
export async function set_filters_from_json() {
    const { TYPES_OF_FOOD, Capitalize, UndersoreString } = await import(
        "./script.js"
    );
