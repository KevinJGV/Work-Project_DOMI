...
export async function initializeDynamicContent() {
    const { FOOD_JSON, TYPES_OF_FOOD, Capitalize } = await import("./script.js");
    ...
    TYPES_OF_FOOD.forEach(type_of_food => {
        ...
        for (const CURRENT_FOOD of FOOD_JSON) {
            if (CURRENT_FOOD["type"] === type_of_food){
                ...
                [H4,P].forEach(elem => CARD_FOOD_DATA_TEXT.insertAdjacentElement("beforeend", elem));
                const IMG = document.createElement("img");
                IMG.src = CURRENT_FOOD["image"];
                [CARD_FOOD_DATA_TEXT,IMG].forEach(elem => CARD_FOOD_DATA.insertAdjacentElement("beforeend", elem));
                ...
                [PRICE,BUTTON].forEach(elem => CARD_FOOD_FOOTER.insertAdjacentElement("beforeend", elem));
                [CARD_FOOD_DATA,CARD_FOOD_FOOTER].forEach(elem => CARD_FOOD.insertAdjacentElement("beforeend", elem));
                GRID.insertAdjacentElement("beforeend", CARD_FOOD)
            }
        };
        [SECTION_TITLE,GRID].forEach(elem => SECTION.insertAdjacentElement("beforeend", elem));
